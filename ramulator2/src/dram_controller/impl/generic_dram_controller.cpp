...
class GenericDRAMController final : public IDRAMController, public Implementation {
  ...
  private:
    ...
    int m_cxl_latency_cycles = 0; // New Member for CXL Simulation
  ...
  public:
    void init() override {
      ...
      // Load CXL Latency (in nanoseconds) from config, default 0 (Local DRAM)
      float cxl_latency_ns = param<float>("cxl_latency_ns").desc("One-way CXL Link Latency in ns").default_val(0.0f);
      // Convert ns to cycles roughly. Assuming 1GHz controller clock for simplicity or use tCK if accessible.
      // Ideally we use m_dram->m_timing_vals("tCK_ps"), but dram is not set in init().
      // We will calculate cycles in setup() where m_dram is available.
      m_cxl_latency_cycles = (int)cxl_latency_ns; // Placeholder, fixed in setup()
    ...
    void setup(IFrontEnd* frontend, IMemorySystem* memory_system) override {
      ...
      // Recalculate CXL Cycles based on tCK
      // We access the raw config param again because we need tCK
      float cxl_latency_ns = param<float>("cxl_latency_ns").default_val(0.0f);
      if (cxl_latency_ns > 0) {
          // Assume tCK is roughly 1ns (1GHz) if not easily accessible, 
          // or better: The controller usually runs at the same clock as DRAM command bus.
          // Let's assume 1 cycle = 1ns for simplicity in this prototype unless we query DRAM.
          // In Ramulator2, controller tick matches DRAM command bus rate.
          // For DDR4-2400, tCK=0.833ns. For HBM3-2000, tCK=1ns.
          // We will use a simplified 1ns = 1 cycle for the link for now.
          m_cxl_latency_cycles = (int)(cxl_latency_ns); 
      }
      ...
      if (request_found) {
        ...
        if (req_it->command == req_it->final_command) {
          if (req_it->type_id == Request::Type::Read) {
            // Add Round-Trip CXL Latency (2x) to the read latency
            req_it->depart = m_clk + m_dram->m_read_latency + (m_cxl_latency_cycles * 2);
