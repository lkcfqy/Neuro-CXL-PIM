...
public:
  void init() override { m_ctrl = cast_parent<IDRAMController>(); };
  ...
  void setup(IFrontEnd *frontend, IMemorySystem *memory_system) override {
    ...
    // Try to get rank level size; for HBM which uses pseudochannel, fallback
    // accordingly
    try {
      m_num_ranks = m_dram->get_level_size("rank");
    } catch (const std::out_of_range &) {
      // HBM uses pseudochannel instead of rank
      try {
        m_num_ranks = m_dram->get_level_size("pseudochannel");
      } catch (const std::out_of_range &) {
        m_num_ranks = 1; // Single channel with no sub-levels
      }
    }
