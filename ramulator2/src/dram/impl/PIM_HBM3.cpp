...
class PIM_HBM3 : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, PIM_HBM3, "PIM_HBM3",
                                    "PIM-Enabled HBM3 Device Model")
...
public:
  ...
  inline static const std::map<std::string, std::vector<int>> timing_presets = {
      ...
      //   nRFC  nRFCSB  nREFI  nREFISB  nRREFD  tCK_ps  nMAC
      {"HBM3_2Gbps_PIM",
       {2000, 4, 7, 7, 7, 7,  17, 19,  8,    2,  3, 2,    1, 2,
        2,    3, 3, 4, 3, 15, -1, 160, 3900, -1, 8, 1000, 16}},
  };
  ...
  // Note: Using 'rank' instead of 'pseudochannel' for ramulator2 core
  // compatibility HBM pseudo-channels are mapped as 'rank' for simulation
  // purposes
  ...
  inline static constexpr ImplDef m_commands = {
      "ACT",    "PRE",   "PREA",  "RD",    "WR",    "RDA",
      "WRA",    "REFab", "REFsb", "RFMab", "RFMsb",
      "PIM_MAC" // <--- NEW
  };
  ...
  inline static constexpr ImplDef m_requests = {
      ...
      "all-bank-rfm", "per-bank-rfm", "pim-mac"};
   ...
   *                 Node States
   ***********************************************/
  inline static constexpr ImplDef m_states = {"Opened", "Closed", "PowerUp",
                                              "N/A", "Refreshing"};
...
public:
  struct Node : public DRAMNodeBase<PIM_HBM3> {
    Node(PIM_HBM3 *dram, Node *parent, int level, int id)
        : DRAMNodeBase<PIM_HBM3>(dram, parent, level, id){};
  };
...
#define V(timing) (m_timing_vals(timing))
    populate_timingcons(
        this, {
                  ...
                  /*** Rank (mapped from Pseudo Channel) ***/
                  ...
                  {.level = "bank",
                   .preceding = {"ACT"},
                   .following = {"PIM_MAC"},
                   .latency = V("nRCDRD")}, // Assume Read-like latency to start

                  // 2. PIM Execution Time
                  {.level = "bank",
                   .preceding = {"PIM_MAC"},
                   .following = {"PRE"},
                   .latency = V("nMAC") + V("nRTPS")}, // Wait MAC finish

                  // 3. PIM to PIM (Pipeline or Serial?) -> Let's assume Serial
                  // per bank
                  {.level = "bank",
                   .preceding = {"PIM_MAC"},
                   .following = {"PIM_MAC"},
...
#undef V
  ...
  void set_actions() {
    ...
    m_actions[m_levels["channel"]][m_commands["PREA"]] =
        Lambdas::Action::Channel::PREab<PIM_HBM3>;
    // Bank actions
    m_actions[m_levels["bank"]][m_commands["ACT"]] =
        Lambdas::Action::Bank::ACT<PIM_HBM3>;
    m_actions[m_levels["bank"]][m_commands["PRE"]] =
        Lambdas::Action::Bank::PRE<PIM_HBM3>;

    // PIM Action (No state change, just access)
    m_actions[m_levels["bank"]][m_commands["PIM_MAC"]] =
        [](Node *node, int cmd, int target_id, int clk) {};
  };

  void set_preqs() {
    m_preqs.resize(m_levels.size(),
                   std::vector<PreqFunc_t<Node>>(m_commands.size()));
    // Use Rank lambdas since we now have rank in the hierarchy
    m_preqs[m_levels["rank"]][m_commands["REFab"]] =
        Lambdas::Preq::Rank::RequireAllBanksClosed<PIM_HBM3>;
    m_preqs[m_levels["bank"]][m_commands["REFsb"]] =
        Lambdas::Preq::Bank::RequireBankClosed<PIM_HBM3>;
    m_preqs[m_levels["bank"]][m_commands["RD"]] =
        Lambdas::Preq::Bank::RequireRowOpen<PIM_HBM3>;
    m_preqs[m_levels["bank"]][m_commands["WR"]] =
        Lambdas::Preq::Bank::RequireRowOpen<PIM_HBM3>;

    // PIM Preq
    m_preqs[m_levels["bank"]][m_commands["PIM_MAC"]] =
        Lambdas::Preq::Bank::RequireRowOpen<PIM_HBM3>;
  };
  ...
  void set_rowhits() {
    ...
    m_rowhits[m_levels["bank"]][m_commands["RD"]] =
        Lambdas::RowHit::Bank::RDWR<PIM_HBM3>;
    m_rowhits[m_levels["bank"]][m_commands["WR"]] =
        Lambdas::RowHit::Bank::RDWR<PIM_HBM3>;
    m_rowhits[m_levels["bank"]][m_commands["PIM_MAC"]] =
        Lambdas::RowHit::Bank::RDWR<PIM_HBM3>;
  }
  ...
  void set_rowopens() {
    ...
    m_rowopens[m_levels["bank"]][m_commands["RD"]] =
        Lambdas::RowOpen::Bank::RDWR<PIM_HBM3>;
    m_rowopens[m_levels["bank"]][m_commands["WR"]] =
        Lambdas::RowOpen::Bank::RDWR<PIM_HBM3>;
    m_rowopens[m_levels["bank"]][m_commands["PIM_MAC"]] =
        Lambdas::RowOpen::Bank::RDWR<PIM_HBM3>;
  }
